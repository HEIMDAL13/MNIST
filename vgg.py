...
class VGG(nn.Module):
    ...
    def __init__(self, opt, num_classes=10):
        ...
        self.features = make_layers(cfg[opt.vgg_model])
        self.fc1 = nn.Sequential(nn.Linear(512, 1024), nn.ReLU(), nn.Dropout(p=opt.drop1))
        self.fc2 = nn.Sequential(nn.Linear(1024, 1024), nn.ReLU(), nn.Dropout(p=opt.drop2))
...
class VGG_LSTM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.opt = opt
        self.features = make_layers(cfg[opt.vgg_model])
        self.lstm_pose_lr = nn.LSTM(input_size=32, hidden_size=self.n_hidden,
        ...
        self.fc1 = nn.Sequential(nn.Linear(512, 1024), nn.ReLU(), nn.Dropout(p=opt.drop1))
        self.fc3 = nn.Linear(self.n_hidden*4, 10)
        self._initialize_weights()
